...
The university week runs Sunday-Thursday (Friday-Saturday are weekends). All times are 24h format.`,
    ...
    if (isRateLimit) {
      return new Response(
        JSON.stringify({ error: 'AI rate limit reached. Please wait a minute and try again.' }),
        { status: 429, headers: { 'Content-Type': 'application/json' } }
      );
    }
    
    return new Response(
      JSON.stringify({ error: error?.message || 'An unexpected error occurred with the AI service.' }),
